...
def best_first_search(
        ...
        with Dojo(theorem, hard_timeout=timeout) as (dojo, init_state):
            ...
            for iteration in trange(max_iters):
                ...
                for step, score in zip(step_cands, step_scores):
                    ...
                    # iter_info['result'].append(result)
                    iter_info['step'].append(step)
                    ...
                    if isinstance(result, ProofFinished):
                        iter_info['type'].append('ProofFinished')
                        ...
                        iter_info['type'].append('TacticState')
                        ...
                        iter_info['type'].append('else')

                iters_info.append(iter_info)
    ...
    return attempt_results, iters_info
...
def _load_data(dataset_name, dataset_path):
    if 'minif2f' in dataset_name:
        ...
        print('is_available_in_cache',is_available_in_cache(repo))
...
def make_output_dir(output_dir,job):
    # dt = datetime.now().strftime("%d-%m-%Y-%H-%M")
    output_dir = os.path.join(output_dir, job)
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    return output_dir
...
if __name__ == '__main__':
    ...
    output_dir = make_output_dir(args.output_dir,args.job)
    output_file = os.path.join(
        output_dir,
        'results__%s__%s.json' % (args.model_name.replace('/', '_'), args.shard)
    )
    ...
    for example in tqdm(data, total=len(data)):
        ...
        attempt_results, iters_info = best_first_search(
